...
import { buildCoachSystemPrompt, buildCoachUserPrompt, buildSiblingCoachSystemPrompt, buildSiblingCoachUserPrompt } from './prompts.mjs'
import { buildAnalyticsSummary, buildSessionPrepSummary } from './analytics.mjs'
import { shieldSituation, getCrisisResponse, getScopeDisclosure, DEFAULT_LOCALE } from './safety-guard.mjs'
...
const SURFACE_SCOPE = Object.freeze({
  parent: 'ParentScript is a parenting support tool, not therapy, counseling, or medical advice. It is not a crisis service. If you or your child are in crisis, call 988, 911, the Childhelp National Child Abuse Hotline (1-800-422-4453), or text HOME to 741741.',
  sibling: 'SiblingSupport is a peer-support tool, not counseling, therapy, or medical advice. It is not a crisis service. If you or your sibling are in crisis, call or text 988, call 911, or call the Childhelp National Child Abuse Hotline (1-800-422-4453).',
})
...
app.use('/api/coach', ipRateMiddleware('coach', envLimit('PARENTSCRIPT_RATE_LIMIT_COACH_PER_MIN', 20)))
app.use('/api/stripe', ipRateMiddleware('stripe', envLimit('PARENTSCRIPT_RATE_LIMIT_DEFAULT_PER_MIN', 30)))
...
app.post('/api/stripe/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (!stripeKey || !webhookSecret) return res.status(503).json({ error: 'Stripe not configured.' })
  ...
  if (idempotencyError && (idempotencyError.message.includes('duplicate') || idempotencyError.code === '23505')) {
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        const plan = Object.entries(STRIPE_PRICE_MAP).find(([_, v]) => v === priceId)?.[0] || 'solo'
      ...
      case 'invoice.payment_failed': {
        ...
        const { data: subRow } = await admin.from('subscriptions').select('id').eq('stripe_subscription_id', invoice.subscription).maybeSingle()
...
app.post('/api/coach', async (req, res) => {
  ...
  if (wantsStream) {
    ...
    res.write('data: ' + JSON.stringify({ error: 'AI coach is not configured on this server.' }) + '\n\n')
    res.end()
    return
  }
...
app.post('/api/analytics', async (req, res) => {
  ...
  try {
    if (type === 'practicePatterns') {
      if (!clientId) return res.status(400).json({ error: '`clientId` required for practicePatterns.' })
      if (isTherapist) {
        const { data: client } = await adminClient
          .from('clients').select('id, therapist_id').eq('id', clientId).maybeSingle()
...
function parseLabeledLine(line) {
  ...
  if (!text) return null
  switch (label) {
    case 'EMPATHY':    return { type: 'empathy', text }
    case 'STEP1':      return { type: 'step', index: 0, text }
    case 'STEP2':      return { type: 'step', index: 1, text }
    case 'STEP3':      return { type: 'step', index: 2, text }
    case 'SAFETY':     return { type: 'safety', text }
...
async function streamAnthropic(sysPrompt, userPrompt, res) {
  ...
  try {
    ...
    res.write('data: ' + JSON.stringify({ error: 'Could not reach AI service. Check your connection.' }) + '\n\n')
    res.end()
    return
  }
...
function parseCoachText(text) {
  ...
  for (const line of lines) {
    ...
    if (!evt) continue
    switch (evt.type) {
      case 'empathy':    empathyText = evt.text; break
      case 'step':       steps[evt.index] = evt.text; break
      case 'safety':     safetyNote = evt.text; break
  ...
  return {
    ...
    safety_note: safetyNote || 'If your child is in immediate danger of hurting themselves or others, call 911.',
    crisis_response: false,
  }
}

const DEFAULT_DISCLAIMER = 'This is general guidance from AI, not medical or therapeutic advice. Your therapist knows your child best. For emergencies, call 911; for crisis support, call or text 988.'
