...
it('user-prompt builder surfaces the right framing', () => {
  expect('parent user prompt frames the situation as parent', buildCoachUserPrompt('test').toLowerCase().includes('parent is facing'))
  expect('sibling user prompt frames as sibling', buildSiblingCoachUserPrompt('test').toLowerCase().includes('sibling'))
})
...
it('crisis payload uses surface-specific scope text (per-surface, not per-locale)', () => {
  ...
  expect('parent scope mentions ParentScript', parentScopePayload.safetyNote.includes('ParentScript'))
  expect('sibling scope mentions SiblingSupport', siblingScopePayload.safetyNote.includes('SiblingSupport'))
  expect('crisis steps[1] is the same locale text in both', parentScopePayload.steps[1] === siblingScopePayload.steps[1])
  expect('crisis steps[1] is en-US verbatim', parentScopePayload.steps[1] === CRISIS_RESPONSE_EN_US)
})
