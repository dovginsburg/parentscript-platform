...
export async function buildAnalyticsSummary(adminClient, clientId) {
  ...
  const { data: logs, error } = await adminClient
    .from('practice_logs')
    .select(
      'id, went_how, practiced_at, reflection_tags, skill_id, skills:skill_id ( id, title, slug, level )'
    )
...
export async function buildSessionPrepSummary(adminClient, therapistId /*, _therapistIdParam */) {
  ...
  const { data: logs, error: lErr } = await adminClient
    .from('practice_logs')
    .select(
      'id, client_id, went_how, practiced_at, reflection_tags, skill_id, skills:skill_id ( id, title, slug, level )'
    )
    .in(
      'client_id',
