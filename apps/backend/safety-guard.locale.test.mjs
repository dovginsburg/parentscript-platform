...
it('getScopeDisclosure(en-US) returns the Mira-approved scope text', () => {
  ...
  expect(
    'mentions parenting support tool',
    getScopeDisclosure('en-US').includes('parenting support tool')
  );
  expect('mentions not therapy', getScopeDisclosure('en-US').includes('not therapy'));
  expect(
    'mentions not a crisis service',
    getScopeDisclosure('en-US').includes('not a crisis service')
  );
});
...
it('locale registry entries are frozen (no runtime mutation)', () => {
  ...
  expect(
    'mutation throws (frozen) — or at least does not stick',
    mutationThrew || entry.crisis === CRISIS_RESPONSE_EN_US
  );
});
