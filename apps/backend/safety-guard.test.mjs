...
it('shieldSituation returns verbatim response for triggered input', () => {
  ...
  expect(
    'contains 988',
    r.response.steps.some(s => s.includes('988'))
  );
  expect(
    'contains 911',
    r.response.steps.some(s => s.includes('911'))
  );
  expect(
    'contains Childhelp',
    r.response.steps.some(s => s.includes('1-800-422-4453'))
  );
  expect(
    'contains DV hotline',
    r.response.steps.some(s => s.includes('1-800-799-SAFE'))
  );
});
...
it('SCOPE_DISCLOSURE_TEXT mentions tool vs. therapy', () => {
  expect(
    ...
    SCOPE_DISCLOSURE_TEXT.includes('parenting support tool')
  );
