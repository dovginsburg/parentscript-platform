      ...
      {brief && expanded && (
        <div className="px-6 pb-6 border-t border-gray-100 pt-4">
          ...
          <div className="grid grid-cols-3 gap-3 mb-5">
            ...
            <div
              className={`rounded-lg px-4 py-3 text-center ${brief.caseload.noRecentPractice > 0 ? 'bg-amber-50' : 'bg-gray-50'}`}
            >
              <p
                className={`text-2xl font-black ${brief.caseload.noRecentPractice > 0 ? 'text-amber-700' : 'text-gray-900'}`}
              >
                ...
                {brief.clients.map(client => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      >
                        {client.last7Count > 0
                          ? `${client.last7Count} this week`
                          : formatDaysAgo(client.daysSinceLastLog)}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 mb-2">
                      {client.last30Count} log{client.last30Count !== 1 ? 's' : ''} in 30 days
                      {client.lastLogAt && <> · Last: {formatDaysAgo(client.daysSinceLastLog)}</>}
                    </p>
