...
export default function ClientList() {
  ...
  useEffect(() => { loadClients() }, [therapist])

  async function handleAddClient(e: React.FormEvent) {
    e.preventDefault()
    if (!therapist || !newLabel.trim()) return
    const trimmed = newLabel.trim()
    if (trimmed.length < 1) { setAddError('Label is required.'); return }
    if (trimmed.length > 120) { setAddError('Label must be 120 characters or fewer.'); return }
    setAdding(true)
    setAddError(null)
      ...
      <main className="max-w-5xl mx-auto px-6 md:px-10 py-8">
        ...
        {atClientLimit && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6 text-sm text-amber-800">
            ...
            <p>
              Your <strong>{planName}</strong> plan allows {planName === 'free' ? '0 clients' : planName === 'solo' ? 'up to 25 clients' : 'unlimited clients'}.
              {planName === 'free' ? (
                <> <Link to="/pricing" className="text-brand-700 hover:underline font-medium">Upgrade to a paid plan</Link> to start adding clients.</>
              ) : (
                <> <Link to="/billing" className="text-brand-700 hover:underline font-medium">Review your billing</Link> to resolve any subscription issues.</>
              )}
            </p>
          </div>
        )}
            ...
            {!labelWarningAccepted ? (
              <>
                <div className="bg-amber-50 border border-amber-300 rounded-lg p-4 mb-4">
                  ...
                  <p className="text-sm text-amber-700">
                    Do <strong>not</strong> use a real name, date of birth, or any identifying information.
                    Use a non-identifying label you'll recognize (e.g., "Client 042", "Red file", "Tuesday family").
                    You hold the label→patient mapping outside this app.
                  </p>
                </div>
              ...
              <form onSubmit={handleAddClient} className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => { setShowAddForm(false); setLabelWarningAccepted(false) }}
                    className="text-sm text-gray-500 hover:text-gray-700 min-h-tap flex items-center"
                  >
                    Cancel
                  </button>
