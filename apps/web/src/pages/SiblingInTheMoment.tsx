    ...
    <div className="min-h-dvh bg-ps-bg-soft flex flex-col">
      ...
      <header className="bg-ps-bg border-b border-ps-border px-4 md:px-8 pt-safe-top pb-4 flex items-center justify-between">
        ...
        <Link to="/sibling" className="flex items-center gap-2 text-sm font-bold text-ps-text">
          <span
            aria-hidden="true"
            className="w-6 h-6 grid place-items-center bg-ps-accent text-white rounded-md text-xs font-bold"
          >
            S
          </span>
          SiblingSupport
          ...
          <>
            <div className="mb-6">
              ...
              <p className="text-ps-text-soft leading-relaxed">
                One tap to get grounded. Type what just happened — even a sentence is enough. The
                coach will help you slow down, listen, and try one thing at a time.
              </p>
            </div>
          ...
          <div>
            ...
            <ol className="space-y-3">
              {(aiResult?.steps || streamedSteps.filter(Boolean) || []).map((step, i) => (
                <li key={i} className="ps-card flex gap-3 items-start">
                  <span
                    aria-hidden="true"
            ...
            {aiMode === 'result' && aiResult && (
              <div className="ps-alert ps-alert-warm mt-6">
                <div className="ps-alert-content">
                  ...
                  <p className="text-sm text-ps-text leading-relaxed">{aiResult.safetyNote}</p>
                </div>
              </div>
            )}
            ...
            {aiMode === 'result' && (
              <div className="mt-6 flex flex-col sm:flex-row gap-3">
                ...
                <Link to="/sibling/safety" className="flex-1 ps-btn ps-btn-secondary ps-btn-lg">
