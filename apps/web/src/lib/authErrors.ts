...
const DEFAULT_MESSAGE = 'Something went wrong. Please try again — if the problem continues, contact support@parentscript.app.'
...
const AUTH_ERROR_PATTERNS: Array<{
  ...
  {
    test: (e) => /row-level security|row level security|RLS/i.test(e.message ?? '') || e.code === '42501',
    message: 'Account setup failed. Please try again — if it keeps happening, contact support.',
    code: 'rls_blocked',
  },
  // Email already registered
  {
    test: (e) => /already registered|already been registered|user already exists/i.test(e.message ?? '') || e.code === 'user_already_exists',
  ...
  {
    test: (e) => /invalid email|email is invalid/i.test(e.message ?? '') || e.code === 'email_address_invalid',
    message: 'That email address looks invalid. Please double-check it.',
    code: 'invalid_email',
  },
  // Weak password (server-side, in case minLength is bypassed)
  {
    test: (e) => /password.*(short|weak|characters)/i.test(e.message ?? '') || e.code === 'weak_password',
    message: 'Password must be at least 8 characters.',
    code: 'weak_password',
  },
  // Rate limiting
  {
    test: (e) => /rate limit|too many requests|over_email_send_rate_limit/i.test(e.message ?? '') || e.code === 'over_email_send_rate_limit' || e.status === 429,
    message: 'Too many attempts. Please wait a minute and try again.',
    code: 'rate_limited',
  },
  // Network / unreachable
  {
    test: (e) => /failed to fetch|networkerror|network request failed/i.test(e.message ?? '') || e.code === 'network_error',
